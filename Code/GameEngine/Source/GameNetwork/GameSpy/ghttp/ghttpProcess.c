...
// BFME2 uses the CRT multibyte duplicate for SDK strdup calls. The retail
// IAT at 0x00BBA5C8 identifies msvcr71.dll!_mbsdup. Other explicit goastrdup
// calls keep their separate SDK helper, so the adaptation stays local.
#include <mbstring.h>
#undef strdup
...
)
{
	...
	// BFME2 rejects an incomplete scan before using statusStringIndex.
	if(rcode != 3)
	{
		connection->completed = GHTTPTrue;
	...
	// Check what we got.
	/////////////////////
	if((majorVersion < 1) ||  // Major version is less than 1.
