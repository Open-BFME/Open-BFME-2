...
/* BFME2 GameSpy Chat callback reconstruction from nitrocaster/GameSpy
 * d1deb2d1a951cf77933dda040b8d311cc09815a7, src/GameSpy/Chat/chatCallbacks.c.
 * Retail has32 callback kinds, without the later UDP relay callback. Its COPY
 * macro relies on the initial parameter-block copy for null string fields.
 * The32-entry switch tables and each complete executable extent are checked
 * independently; table/alignment bytes are excluded from function claims.
...
#define COPY(param)         if(srcParams->param != NULL)\
					...
					/*ERRCON*/		return CHATFalse;\
							...
							} /* Retail already copied the parameter block before deep-copying strings. */
...
static void ciFreeCallbackData(ciCallbackData * data)
{
	...
	/* Retail has32 callback kinds; UDP relay was added in the later SDK. */
