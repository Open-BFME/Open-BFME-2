...
// (0x003D46DB) and 0x00204D87; the method name is descriptive, since no retail
// symbol names it. It ends exactly where the action table begins.
//
// Donor: BFME1 initConditionTemplates (reference/open-bfme-1,
// script_conditions.cpp, b1 0x003307E0). Every donor statement reappears in
// retail in donor order; BFME2 adds 18 templates and the target-only +0x00 int.
// Statements were read from the retail stores, and the donor supplies the
// curTemplate form and the field names.
//
// Retail inlines operator=(const char *) for the first 669 assignments (a
// direct call to StringBase<char>::set, 0x000055F5) and then calls it out of
// line (0x000065B8) for the remaining 246. That cutoff is MSVC 7.1's
// per-function inline budget, so it depends on the IL weight of the whole
// body, including statements the optimizer later deletes. The visible stores
// alone give 675 inlines and reschedule templates 123-127 and 169. Retail
// needs 15 more no-code statements. Two kinds are written here:
//   * the nine indices retail never fills (59, 60, 67, 68, 72, 73, 115, 116,
//     122, the same holes as BFME1) keep their curTemplate selection, as a
//     commented-out block would leave it;
//   * six dead m_numUiStrings pre-writes. These are not recovered source. They
//     stand in for IL that the retail compile carried and that no retail byte
//     reveals. Any six no-code statements anywhere in the body give the same
//     bytes (verified by trial), so their placement carries no meaning.
// The operator= shape does not change the cutoff (member, base, cast, in-class
// all give the same result); routing it through an inline AsciiString::set
// halves it to 338.
...
inline AsciiString &AsciiString::operator=(const char *text)
{
...
void ScriptEngine::initConditionTemplates()
{
	...
	// dead: overwritten below (inline-budget stand-in, see header)
	curTemplate->m_numUiStrings = 0;
	...
	curTemplate = &m_conditionTemplates[189];
	curTemplate->m_internalName = "CAMERA_SCROLL_DISTANCE";
	curTemplate->m_uiName = "Camera_/ Camera scroll distance.";
	curTemplate->m_numParameters = 1;
	curTemplate->m_parameters[0] = 1;
	curTemplate->m_numUiStrings = 2;
	curTemplate->m_uiStrings[0] = "Player has scrolled camera ";
	curTemplate->m_uiStrings[1] = " units.";
	curTemplate = &m_conditionTemplates[190];
	curTemplate->m_internalName = "CAMERA_ROTATE_DISTANCE";
	curTemplate->m_uiName = "Camera_/ Camera rotate distance.";
	curTemplate->m_numParameters = 1;
	curTemplate->m_parameters[0] = 1;
	curTemplate->m_numUiStrings = 2;
	curTemplate->m_uiStrings[0] = "Player has rotated camera ";
	curTemplate->m_uiStrings[1] = " degrees.";
	curTemplate = &m_conditionTemplates[191];
	curTemplate->m_internalName = "CAMERA_ZOOM_DISTANCE";
	curTemplate->m_uiName = "Camera_/ Camera zoom distance.";
	curTemplate->m_numParameters = 1;
	curTemplate->m_parameters[0] = 1;
	curTemplate->m_numUiStrings = 2;
	curTemplate->m_uiStrings[0] = "Player has zoomed camera ";
	curTemplate->m_uiStrings[1] = " units.";
	curTemplate = &m_conditionTemplates[192];
	curTemplate->m_internalName = "CAMERA_RESET";
	curTemplate->m_uiName = "Camera_/ Camera reset.";
	curTemplate->m_numParameters = 0;
	curTemplate->m_numUiStrings = 1;
	curTemplate->m_uiStrings[0] = "Player has reset camera";
	curTemplate = &m_conditionTemplates[13];
	curTemplate->m_internalName = "NAMED_INSIDE_AREA";
	...
	curTemplate = &m_conditionTemplates[200];
	curTemplate->m_internalName = "TYPE_SELECTED";
	curTemplate->m_uiName = "Unit_/ Unit type currently selected.";
	curTemplate->m_numParameters = 1;
	curTemplate->m_parameters[0] = 61;
	curTemplate->m_numUiStrings = 2;
	curTemplate->m_uiStrings[0] = " ";
	...
	curTemplate->m_uiStrings[0] = " ";
	curTemplate->m_uiStrings[1] = " has completed playing.";
	// never filled in retail
	curTemplate = &m_conditionTemplates[59];
	// never filled in retail
	curTemplate = &m_conditionTemplates[60];
	...
	// never filled in retail
	curTemplate = &m_conditionTemplates[67];
	// never filled in retail
	curTemplate = &m_conditionTemplates[68];
	...
	// never filled in retail
	curTemplate = &m_conditionTemplates[72];
	// never filled in retail
	curTemplate = &m_conditionTemplates[73];
	...
	curTemplate->m_uiStrings[3] = " unit or structure of type ";
	curTemplate->m_uiStrings[4] = " in the ";
	curTemplate = &m_conditionTemplates[188];
	curTemplate->m_internalName = "PLAYER_HAS_COMPARISON_UNIT_TYPE_IN_TRIGGER_AREA_COMPLETELY_BUILT";
	curTemplate->m_uiName = "Player_/ Player has (comparison) of completely built unit type in an area.";
	curTemplate->m_numParameters = 5;
	curTemplate->m_parameters[0] = 11;
	curTemplate->m_parameters[1] = 6;
	curTemplate->m_parameters[2] = 0;
	...
	curTemplate->m_uiStrings[1] = " has ";
	curTemplate->m_uiStrings[2] = " ";
	curTemplate->m_uiStrings[3] = " completely built units or structures of type ";
	curTemplate->m_uiStrings[4] = " in the ";
	curTemplate = &m_conditionTemplates[194];
	curTemplate->m_internalName = "PLAYER_ISSUED_FORMATION_ORDER";
	curTemplate->m_uiName = "Player_/ Player has issued formation order";
	curTemplate->m_numParameters = 0;
	curTemplate->m_numUiStrings = 1;
	curTemplate->m_uiStrings[0] = "Player has issued formation order";
	...
	curTemplate->m_uiStrings[0] = " ";
	curTemplate->m_uiStrings[1] = " has ";
	curTemplate = &m_conditionTemplates[193];
	curTemplate->m_internalName = "UNIT_USING_STANCE";
	curTemplate->m_uiName = "Unit_/Is named unit in a stance.";
	curTemplate->m_numParameters = 2;
	curTemplate->m_parameters[0] = 14;
	curTemplate->m_parameters[1] = 68;
	curTemplate->m_numUiStrings = 3;
	curTemplate->m_uiStrings[0] = " Is ";
	curTemplate->m_uiStrings[1] = " in the ";
	curTemplate->m_uiStrings[2] = " stance.";
	...
	curTemplate->m_uiName = "Skirmish Only_/ Player has (condition) threat level in area.";
	curTemplate->m_numParameters = 4;
	curTemplate->m_parameters[0] = 11;
	curTemplate->m_parameters[1] = 6;
	curTemplate->m_parameters[2] = 1;
	...
	curTemplate->m_uiStrings[1] = " has ";
	curTemplate->m_uiStrings[2] = " ";
	curTemplate->m_uiStrings[3] = " threat within area ";
	...
	// never filled in retail
	curTemplate = &m_conditionTemplates[122];
	...
	curTemplate = &m_conditionTemplates[201];
	curTemplate->m_internalName = "IS_GAME_MODE_ACTIVE";
	curTemplate->m_uiName = "GameType/Is game mode active?";
	curTemplate->m_numParameters = 1;
	curTemplate->m_parameters[0] = 10;
	curTemplate->m_numUiStrings = 2;
	curTemplate->m_uiStrings[0] = " Is ";
	curTemplate->m_uiStrings[1] = " game mode active";
	...
	curTemplate = &m_conditionTemplates[184];
	curTemplate->m_internalName = "IS_AREA_ON_FIRE";
	curTemplate->m_uiName = "Map_/Area on fire.";
	curTemplate->m_numParameters = 1;
	curTemplate->m_parameters[0] = 9;
	curTemplate->m_numUiStrings = 2;
	curTemplate->m_uiStrings[0] = " Is any cell within ";
	curTemplate->m_uiStrings[1] = " on fire";
	curTemplate = &m_conditionTemplates[185];
	curTemplate->m_internalName = "LIVING_WORLD_IS_REGION_REF_BOUND";
	curTemplate->m_uiName = "_Region/Is Region Ref bound to a region?";
	curTemplate->m_numParameters = 1;
	curTemplate->m_parameters[0] = 71;
	curTemplate->m_numUiStrings = 2;
	curTemplate->m_uiStrings[0] = " Is ";
	curTemplate->m_uiStrings[1] = " bound to a region";
	curTemplate->m_targetOnly00 = 2;
	curTemplate = &m_conditionTemplates[186];
	curTemplate->m_internalName = "HAS_EVA_EVENT_PLAYED_IN_LAST_N_SECONDS";
	curTemplate->m_uiName = "Player_/Eva event played recently";
	curTemplate->m_numParameters = 2;
	curTemplate->m_parameters[0] = 76;
	curTemplate->m_parameters[1] = 1;
	curTemplate->m_numUiStrings = 3;
	curTemplate->m_uiStrings[0] = " Has ";
	curTemplate->m_uiStrings[1] = " played with in the last ";
	curTemplate->m_uiStrings[2] = " seconds";
	curTemplate = &m_conditionTemplates[187];
	curTemplate->m_internalName = "IS_NUM_OF_UNITS_BELONGING_TO_PLAYER_NEAR_EVA_EVENT_LAST_PLAYED_LOCATION_COMPARISON_INT";
	curTemplate->m_uiName = "Player_/Is number of units belonging to a player near EVA event's last played location greater than, less than, or equal to a fixed value";
	curTemplate->m_numParameters = 5;
	curTemplate->m_parameters[0] = 11;
	curTemplate->m_parameters[1] = 1;
	curTemplate->m_parameters[2] = 76;
	curTemplate->m_parameters[3] = 6;
	curTemplate->m_parameters[4] = 0;
	curTemplate->m_numUiStrings = 5;
	curTemplate->m_uiStrings[0] = " Is the number of units belong to ";
	curTemplate->m_uiStrings[1] = " within ";
	curTemplate->m_uiStrings[2] = " feet of the location where ";
	curTemplate->m_uiStrings[3] = " last played ";
	curTemplate->m_uiStrings[4] = " ";
	curTemplate = &m_conditionTemplates[195];
	curTemplate->m_internalName = "HAS_DELAYED_CARRYOVER_UNIT_OF_TYPE";
	curTemplate->m_uiName = "Unit_/Is there a delayed-carryover unit of type";
	curTemplate->m_numParameters = 2;
	curTemplate->m_parameters[0] = 61;
	curTemplate->m_parameters[1] = 11;
	curTemplate->m_numUiStrings = 3;
	curTemplate->m_uiStrings[0] = " Is there a delayed-carryover object of type ";
	curTemplate->m_uiStrings[1] = " belonging to ";
	curTemplate->m_uiStrings[2] = " still waiting to spawn";
	curTemplate = &m_conditionTemplates[196];
	curTemplate->m_internalName = "TOGGLE_STANCE_SUBMENU_IS_OPEN";
	curTemplate->m_uiName = "Interface_/The toggle stance command sub-menu is open";
	curTemplate->m_numParameters = 0;
	curTemplate->m_numUiStrings = 1;
	curTemplate->m_uiStrings[0] = "The toggle stance command sub-menu is open.";
	curTemplate = &m_conditionTemplates[197];
	curTemplate->m_internalName = "SPELL_STORE_IS_OPEN";
	curTemplate->m_uiName = "Interface_/The spell store is open";
	curTemplate->m_numParameters = 0;
	curTemplate->m_numUiStrings = 1;
	curTemplate->m_uiStrings[0] = "The spell store is open.";
	curTemplate = &m_conditionTemplates[198];
	curTemplate->m_internalName = "OBJECTIVES_SCREEN_IS_OPEN";
	curTemplate->m_uiName = "Interface_/The objectives screen is open";
	curTemplate->m_numParameters = 0;
	curTemplate->m_numUiStrings = 1;
	curTemplate->m_uiStrings[0] = "The objectives screen is open.";
	curTemplate = &m_conditionTemplates[199];
	curTemplate->m_internalName = "PLAYER_IS_IN_PLANNING_MODE";
	curTemplate->m_uiName = "Player_/Player is in planning mode";
	curTemplate->m_numParameters = 1;
	curTemplate->m_parameters[0] = 11;
	curTemplate->m_numUiStrings = 2;
	curTemplate->m_uiStrings[0] = "Player ";
	curTemplate->m_uiStrings[1] = " is in planning mode.";
}
