...
// Declared nothrow. Retail changes the unwind state exactly twice in this
// body, and neither change brackets this call: it pushes _Nameless, calls,
// tests the result byte and stores it, with no state store anywhere in the
// sequence. A call that may throw forces MSVC to open a try-region
// transition around it, which costs the state-2 store, the ebx mask spill
// beside it and a matching state-1 store after the result byte -- the twelve
// bytes that widen the `74 79` at +0x2E into a near jump and push the whole
// tail out of alignment. free is left throwing on purpose: retail DOES write
// the state around the second and third destructors.
template <class CharT, class Traits, class Alloc>
bool __cdecl operator!=(const basic_string<CharT, Traits, Alloc> &x,
	const basic_string<CharT, Traits, Alloc> &y) throw();
