...
__declspec(dllimport) __forceinline void _Initialize_get_float(
		char &plus, char &minus, char &exponent_lower, char &exponent_upper,
		char *)
{
	plus = '+';
	minus = '-';
		...
		ios_base &stream, CharT *)
{
	...
	_Initialize_get_float(
			plus, minus, exponent_lower, exponent_upper, digits);
...
template bool _M_read_float<narrow_iterator, char>(
		narrow_string &, narrow_iterator &, narrow_iterator &, ios_base &, char *);
