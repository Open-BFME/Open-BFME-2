...
//
// /G7 is what matches it. Retail spells every pointer and counter step as
// `add reg, 1` / `sub reg, 1` rather than inc/dec, which is the Pentium 4
// form, and without it Encode comes out 264 bytes against 279 with the
// difference spread across the six step sites in the packet loop.
//
// This tree carries no base64.h,
// so the two prototypes stand here instead of an include.
