// cl: /DNDEBUG /MD /EHsc
// Open-BFME7: the library basic_ifstream<char>(const char*, int) constructor at
// 0x0084C300 under an alias class name: its mangled name is already owned by the
// game-side LobbyUtils.cpp copy (0x0053FB00), which has a different filebuf
// layout, so the ledger needs a distinct spelling for this body.
// STLport 4.5 narrow file-stream constructors taking a file name:
// basic_ifstream_lib0084C300<char>(int fd, int), (const char*, int [, long]) and basic_ofstream<char>(...)
...
// stlport_basic_ifstream_lib0084C300_char_default_ctor.cpp, then the filebuf is opened
...
public:
  basic_ifstream_lib0084C300(int fd, int mode)
      : basic_ios<CharT, Traits>(), basic_istream<CharT, Traits>(0), buf_() {
    this->init(&buf_);
    if (!buf_.open(fd, mode | ios_base::in))
      this->setstate(ios_base::failbit);
  }
  basic_ifstream_lib0084C300(const char *name, int mode)
      : basic_ios<CharT, Traits>(), basic_istream<CharT, Traits>(0), buf_() {
    this->init(&buf_);
    if (!buf_.open(name, mode | ios_base::in))
      this->setstate(ios_base::failbit);
  }
  basic_ifstream_lib0084C300(const char *name, int mode, long protection)
      : basic_ios<CharT, Traits>(), basic_istream<CharT, Traits>(0), buf_() {
    this->init(&buf_);
...
template basic_ifstream_lib0084C300<char, char_traits<char> >::basic_ifstream_lib0084C300(const char *, int);
}
