// ?first@Rva000427195@@QAEPAXPAVRva000411084@@@Z
...
// ?first@Rva000427195@@QAEPAXPAVRva000411084@@@Z
void *Rva000427195::first(Rva000411084 *iter)
{
    ...
    while (m_beginBuckets[i] == 0)
    {
        ++i;
        if (i >= (unsigned)(((char *)m_endBuckets - (char *)m_beginBuckets) >> 2))
        {
            iter->m_current = 0;
            iter->m_owner = this;
            return iter;
    ...
    iter->m_current = m_beginBuckets[i];
    iter->m_owner = this;
    return iter;
}
