...
//
// ??0RubbleRiseUpdate@@QAE@PAVThing@@PBVModuleData@@@Z, retail
// 0x004A4CC7, 139 bytes. Behavior-side ctor completing the
// RubbleRiseUpdate file-unit (behavior instance factory rowed at
// 0x24EDCF news 0x48 with this pinned 2-arg ctor as its sole caller;
// poolkey rowed at 0x4A4C82; ModuleData proc 0x4A5486 plus ModuleData
// factory 0x2566F6 plus ModuleData ctor 0x256653 all rowed).
//
// Shape follows the StructureCollapseUpdate behavior ctor precedent over
// the same rowed UpdateModule base 0x253390: two-phase +0x20 slot
...
// fldz to retail xorps+movss, DemoTrap precedent). Member names follow
// the Bloodthirsty donor (m_object at +0x08); the frame arg is the
// 0x3FFFFFFF never-wake literal. /GX for the EH prologue plus the single
// mid-body state store; the declared-only base dtor arms it with no
// emitted code. Zero new pins (base ctor resolves via its row;
	...
	: UpdateModule(thing, moduleData)
{
	...
	*slot20 = (int)0x00C1C780;
	float *cooldownSlot = &m_cooldown44;
	int zero = 0;
