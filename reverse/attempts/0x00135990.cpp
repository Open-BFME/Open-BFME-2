...
// partial score=0.8831775700934579 date=2026-09-07
...
// The embedded interface grants Camera access only for this unfinished experiment.
/*
...
bool CameraClass::Cull_Box(const AABoxClass & box) const
{
	...
	for (int i=0; i<6; ++i) {
		...
		Vector3 far_extent;
		get_far_extent(plane.N,box.Extent,&far_extent);
		Vector3 near_point=box.Center-far_extent;
		if (Vector3::Dot_Product(plane.N,near_point)-plane.D>CollisionMath::COINCIDENCE_EPSILON) return true;
	}
	return false;
}
