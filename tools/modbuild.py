...
BASELINE = ROOT / "baselines/bfme2/workshop-vanilla-1.06/files/game.dat"
OUT = ROOT / "build/mods/game.dat"
DIST = ROOT / "mods/dist"
...
UNSHIPPED = {
    ...
    "033-retrytime": (build_retrytime,
                      "measured GREEN at 80ms round trip (freeze p50 1703->427ms, no "
                      "desync, traffic-neutral) and it also repairs 031-earlysend's p95 "
                      "regression under loss -- but duplicate delivery is still untested. "
                      "At 80ms a 400ms timer never fires before the ack, so nothing run "
                      "so far exercises a command arriving twice, and a lockstep command "
                      "executed twice is a desync. Promote when 150ms+ round trip is green"),
}
...
def main():
    ...
    if a.dist:
        ...
        manifest = {
            ...
            "note": "Retail game.dat with a .bfmemod code cave appended and "
