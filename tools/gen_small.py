...
def select_imports(entries, read, imports, blacklist, claimed, batch_source, limit):
    """The next named-IAT `FF 25` thunks with a representable C import name."""
    picked = []
    for rva, size, name in entries:
        if size != 6 or rva in blacklist:
            continue
        slot = iat_slot(read(rva, 6))
        imported = imports.get(slot) if slot is not None else None
        # The conversion gate verifies the import identity as well as copied
        # DIR32 bytes. A synthetic `i_<rva>` dllimport would reproduce the
        # instruction but name a different import, and non-identifier exports
        # need a real declaration rather than an invented alias. The compiler
        # runtime's _CxxThrowException also has a conflicting intrinsic
        # declaration; leave that special entry for a dedicated ABI declaration.
        if (imported is None or not imported.name or not re.fullmatch(
                r"[A-Za-z_][A-Za-z0-9_]*", imported.name)
                or imported.name == "_CxxThrowException"):
            continue
        owner = claimed.get(rva)
        if owner is not None and owner != batch_source:
            continue
        picked.append((rva, slot, name, imported))
        if len(picked) == limit:
            break
    return picked
...
def render_imports(picked, batch, limit):
    ...
    imported_names = sorted({imported.name for _, _, _, imported in picked})
    lines += [f'extern "C" __declspec(dllimport) void {name}();'
              for name in imported_names]
    lines.append("")
    lines += [f"void ji_{rva:08x}() {{ {imported.name}(); }}"
              for rva, _, _, imported in picked]
    return "\n".join(lines) + "\n"
...
def cmd_gen_imports(args):
    ...
    imports = read_imports(B.EXE.read_bytes())
    blacklist = load_blacklist()
    claimed, claimed_names, _ = load_claims()

    source_path, pending_path = import_paths(args.batch)
    source_rel = source_path.relative_to(ROOT).as_posix()
    picked = select_imports(entries, read, imports, blacklist, claimed, source_rel, args.limit)
    if not picked:
        print(f"gen-imports: batch {args.batch}: 0 new — no claimable import thunks left")
        return

    rows = []
    for rva, slot, target_name, imported in picked:
        ...
        rows.append(format_row(name, rva, 6, source_rel,
                               f"gen-import;slot=0x{slot:08X};dll={imported.dll};"
                               f"import={imported.name};target={target_name}"))
