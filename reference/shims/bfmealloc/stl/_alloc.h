...
// BFME SHIM: the byte allocator the whole engine funnels through. It is a
// static __cdecl taking (bytes, hint) and lives at 0x000307F0, where it
// forwards to the game allocator table at 0x00DE0404 with memory class 3.
// Declared before the primary template so the inline allocate above can name
// it; MSVC does not instantiate that body until the container is used, so an
// incomplete type here is enough.
template <class _Tp> class allocator;
_STLP_TEMPLATE_NULL class allocator<char>;
...
# endif
 ...
 # endif    
  ...
  // BFME SHIM: retail does not reach __sgi_alloc at all. Every allocating
  // container in game.dat funnels through one raw byte allocator, called as
  ...
  void deallocate(pointer __p, size_type __n) const {
    _STLP_ASSERT( (__p == 0) == (__n == 0) )
      if (__p != 0) ::free((void*)__p);
  }
  // backwards compatibility
  void deallocate(pointer __p) const {  if (__p != 0) ::free((void*)__p); }
...
// BFME SHIM: allocator<char> is where the out-of-line body actually lives.
// Everything else folds onto it - the linker's identical-COMDAT pass collapses
// every instantiation of this forwarding shape to the same address, which is
// why the narrow and wide allocators share 0x000307F0 in the image.
_STLP_TEMPLATE_NULL
...
# endif
  allocator() _STLP_NOTHROW {}
  allocator(const allocator<char>&) _STLP_NOTHROW {}
  ~allocator() _STLP_NOTHROW {}
  pointer address(reference __x) { return &__x; }
  const_pointer address(const_reference __x) const { return &__x; }
  static char* allocate(size_type __n, const void* __p);
  void deallocate(pointer __p, size_type) const { if (__p != 0) ::free((void*)__p); }
  void deallocate(pointer __p) const { if (__p != 0) ::free((void*)__p); }
  size_type max_size() const _STLP_NOTHROW { return size_t(-1); }
  void construct(pointer __p, const char& __val) const { _STLP_STD::_Construct(__p, __val); }
  void destroy(pointer __p) const { _STLP_STD::_Destroy(__p); }
};
