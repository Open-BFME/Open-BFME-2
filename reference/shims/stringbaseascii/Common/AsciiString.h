#pragma once
// campaignmanagerascii's AsciiString with one change: str() is inlined.
//
// Retail reaches NameKeyGenerator::nameToKey through its const char * overload
// -- the vendored header's nameToKey(const AsciiString&) is an inline that
// forwards to name.str() -- and inlines str() at the call site as
// `mov eax,[esp]; test eax,eax; je +5; add eax,8; jmp +5; mov eax,offset ""`.
// Delegating to StringBase<char>::str(), which is declared and never defined,
// emits a call there instead and every menu Init misses by those bytes.
//
// Kept separate from campaignmanagerascii rather than changed in place:
// that shim carries sixty-odd landed rows in UserPreferences.cpp alone, and
// a TU that wants the out-of-line str() should keep getting it.
...
public:
    ...
    // Inlined rather than delegated. StringBase<char>::str() is declared and
    // not defined, so delegating emits a call; retail inlines the whole thing
    // as the test / add-8 / else-empty-string sequence at every menu
    // nameToKey site, which is what the 8-byte StringBase header buys.
