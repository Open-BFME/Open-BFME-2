// Scoped BFME2 StreakLine header derived from the sweep RenderObj header.
// Only indexed-factor slots 23..25 and the collision getter are reconciled.
// Other inherited field
// offsets and overload ordering remain unverified; vtable contents are not a
// completion claim. The existing base size/layout is otherwise retained.
/*
...
#define GET_CONTAINER_INLINE
	...
	// BFME2 StreakLine-only interface reconciliation. Retail slots23/24 are
...
//	virtual void					Set_VisibleWithCheatSpy(int onoff)								{ if (onoff) { Bits |= IS_VISIBLE|0x80; } else { Bits &= ~IS_VISIBLE; } }
	...
	virtual int						Get_Collision_Type(void) const											{ return (Bits & 0x00000FFEu) | COLL_TYPE_ALL; }
	// Retail slot 120 / VA 0x46D014 masks bits 1..11 and always enables
	// COLL_TYPE_ALL (bit 0); the complete 11-byte getter is independently exact.
...
protected:
	...
	float								_bfme_unk_8c;
	float _bfme_indexed_factors[2]; // retail0x90/0x94; preserves inherited layout
	float								_bfme_unk_98;
