// BFME2 StreakRenderer layout: 0x4C bytes; reconstructed from ctor742F90,
// assignment743100 and copy ctor746DE0. Texture ownership is established by
// the copy ctor unwind action at7AB6C0 (RefCountPtr destructor17098D).
// Derived from the pinned GeneralsMD header below; restored UV fields and
// changed the four-byte texture member to its reference-counted owner.
/*
...
public:
	...
	TextureClass *		Peek_Texture(void) const								{ return Texture.Peek(); }
...
typedef char StreakRendererRetailSize[(sizeof(StreakRendererClass) == 0x4C) ? 1 : -1];
