...
// BFME's TunnelTracker has ONE vptr, not two: its destructor @0xF8A40
// stores a single vtable pointer at [this+0], and every member sits one
// dword lower than the ZH layout - xfer @0xF8E20 walks +4, +8, +0xC,
// +0x10, +0x14, +0x18 and +0x1C in declaration order, isValidContainerFor
// @0xF9170 reads m_containListSize at +0x10, and updateNemesis @0xF90D0
// writes m_curNemesisID/m_nemesisTimestamp at +0x18/+0x1C. That makes
// sizeof 0x20, which is exactly the operator new argument in
// CaveSystem::registerNewCave @0x378970. Dropping the polymorphic Snapshot
// base reproduces it; crc/xfer/loadPostProcess stay virtual on the class
// itself so they still occupy vtable slots.
class TunnelTracker : public Snapshot
{
	MEMORY_POOL_GLUE_WITH_USERLOOKUP_CREATE( TunnelTracker, "TunnelTracker" );

public:
	TunnelTracker();
	// provided by BFME's merged pooled-snapshot base, not by a second base class
	void deleteInstance();
	// contain list access
