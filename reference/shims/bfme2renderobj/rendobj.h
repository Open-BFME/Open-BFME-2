// BFME2 RenderObj layout used only by the reconstructed base-member TUs.
// Exact constructor/copy access patterns establish a 0xC4 object, Scale@48,
// Sphere@4C, Box@5C, Scene@78, Container@7C, UserData@80 and indexed floats@90/94.
// Unknown fields retain neutral names. Two duplicate late native-size virtual
// declarations are removed, placing cache update at the observed slot127.
// This does not claim all inherited method names or complete vtable contents.
/*
...
#define GET_CONTAINER_INLINE
	...
	virtual void _bfme_set_indexed_factor(int index, float value) { _bfme_indexed_factors[index] = value; }
	virtual float _bfme_get_indexed_factor(int index) const { return _bfme_indexed_factors[index]; }
	// Retail slot25 / RVA13BC60 returns the product, before Notify_Added.
	// This extra slot accounts for the subsequent virtual-call displacement.
   ...
   virtual void               Update_Obj_Space_Bounding_Volumes(void)								{ };
	...
	virtual void					_bfme_set_float_84(float v)													{ _bfme_float_84 = v; }
	virtual float					_bfme_get_float_84(void) const												{ return _bfme_float_84; }
...
//	virtual void					Set_Texture_Reduction_Factor(float trf);
	...
	const float						Get_ObjectScale( void ) const { return ObjectScale; };
 	void							Set_ObjectColor(unsigned int color);	//the color that was used to modify the asset for player team color (for Generals). -MW
...
//	virtual void					Set_VisibleWithCheatSpy(int onoff)								{ if (onoff) { Bits |= IS_VISIBLE|0x80; } else { Bits &= ~IS_VISIBLE; } }
   ...
   virtual bool					Is_Complete(void)																{ return false; }
	...
	float					Get_Native_Screen_Size(void) const;
	void					Set_Native_Screen_Size(float screensize);
...
protected:
    ...
    unsigned long _bfme_unk_14;                    // 0x14; collision input mask
    Matrix3D Transform;                            // 0x18
    float ObjectScale;                            // 0x48; scales sphere radius
    mutable SphereClass CachedBoundingSphere;      // 0x4C
    mutable AABoxClass CachedBoundingBox;           // 0x5C
    mutable bool IsTransformIdentity;              // 0x74
    SceneClass *Scene;                             // 0x78
    RenderObjClass *Container;                     // 0x7C
    void *User_Data;                               // 0x80
    float _bfme_float_84;                        // 0x84; application meaning unknown
    unsigned long _bfme_unk_88;                    // 0x88; scene token, meaning unknown
    int _bfme_unk_8c;                              // 0x8C; initialized -1
    float _bfme_indexed_factors[2];                 // 0x90 / 0x94
    int _bfme_unk_98;                              // 0x98; initialized -1
    char _bfme_opaque_9c[32];                       // 0x9C; bytewise copied
    bool _bfme_unk_bc;                             // 0xBC
    bool _bfme_unk_bd;                             // 0xBD
    void *_bfme_unk_c0;                            // 0xC0; copied, no ownership inferred
