...
protected:
  ...
  enum
  {
	...
	// BFME keeps the scalars up front and pushes the two big character buffers to
	// the end of the object. parsePrerequisites (0x001485B0) reads m_loadType at
	// [ini+8], where the Zero Hour order lands it at [ini+0x2010] -- 0x2000 of
	// that is m_readBuffer sitting between m_file and everything else.
