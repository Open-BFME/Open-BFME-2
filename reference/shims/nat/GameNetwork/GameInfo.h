...
// TU-scoped stand-in for the real GameNetwork/GameInfo.h, carrying two BFME
// differences in GameSlot that three independent retail bodies agree on:
//
//   m_IP   at slot+0x30, where this reference header puts it at +0x2C
//          (NAT::gotInternalAddress 0x00670D70 reads and writes it there)
//   m_port at slot+0x34 and m_nat at slot+0x38 -- so GameSlotConnectInfo leads
//          with the port, where the reference leads with the behavior
//          (NAT::getSlotPort 0x00670C60, already matched, reads the port as a
//          word at +0x34; NAT::sendMangledSourcePort 0x00671A90 reads the
//          behavior at +0x38)
//
// Those hold together only if four bytes sit between m_name and m_IP. What
// occupies them is not pinned -- an eight-byte UnicodeString would do it, and so
// would an added member -- so they are modelled as padding rather than guessed
// at. Scoped to this TU via a private /Ireference/shims/nat include.
...
{
	UnsignedShort m_port;															///< port number, first in BFME
...
protected:
	...
	UnsignedInt m_padBeforeIP;												///< unpinned; see the note at the top
