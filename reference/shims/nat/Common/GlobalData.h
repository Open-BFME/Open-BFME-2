...
// TU-scoped stand-in for Common/GlobalData.h, carrying one BFME offset.
//
// BFME's GlobalData is not the reference's with a single insertion -- the four
// members pinned so far sit at two different deltas, and one pair is closer
// together than the reference's, so fields were both added and removed. Building
// the real member list is a separate job. What this shim does instead is far
// narrower: nat.cpp reads exactly two GlobalData members, and only one of them
// appears in code that is matched, so 900 bytes of padding in front of that one
// member puts it where retail has it.
//
//   m_firewallPortAllocationDelta   reference +0x79C -> BFME +0xB20
//
// Pinned by NAT::processManglerResponse (0x00671CA0), which reads it as a word.
...
public:
  ...
  Int   m_statsInterval;       ///< if so, how many is N?
	...
	// Puts m_firewallPortAllocationDelta at +0xB20, where retail has it.
	char m_padToBFMEFirewallDelta[0xB20 - 0x79C];
