...
// TU-scoped stand-in for Common/AsciiString.h, scoped through
// /Ireference/shims/nat. See the note on the constructor below.
...
// -----------------------------------------------------
// Declared, never defined here. Retail's AsciiString(const char *) is the thin
// out-of-line body at 0x00888BC0 and releaseBuffer is 0x00887940 -- the same two
// the BFMERetailAsciiString shims elsewhere in the tree call. Leaving them
// out-of-line is what lets a by-value AsciiString argument be constructed
// directly in its pushed stack slot (push ecx; mov ecx,esp; call ctor), which is
// how retail passes format's V1 parameter. With the reference's inline bodies
// the compiler has to materialise the temporary in a frame local first, and no
// amount of source rearrangement gets those bytes back.
