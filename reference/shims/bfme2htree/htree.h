...
*/
// BFME2 has no W3DMPO base. Its owned sequence and three trailing words
// extend the known BFME1 prefix to 0x34 bytes. The actual copy constructor
// writes through +0x30; its caller allocates 0x34. Unknown field names are
// deliberately neutral; no behavior for those fields is reconstructed here.
...
private:
	...
	PivotClass *		Pivot;
	float					ScaleFactor;
	unsigned int OwnedSequence1C[3];
...
typedef char BFME2HTreeSizeMustBe52[(sizeof(HTreeClass) == 52) ? 1 : -1];
