...
private:
	...
	// ---- BFME Object layout, reconstructed from retail pins -------------
	// Retail agrees with this header for most of the class: rows already
	// byte-matching in Object.cpp pin +0x124, +0x12C, +0x18C, +0x190, +0x194,
	// +0x1A0, +0x1A8, +0x1F0 and +0x23C unchanged. Only three fields move, and
	// they RELOCATE rather than shift - padding in front of them is provably
	// wrong because it would drag the pinned fields with them.
	//   m_disabledMask   +0x130 -> +0x1A4  (doSpecialPowerUsingWaypoints @0x1C3850)
	//   m_triggerInfo    +0x240 -> +0x2D8  (isInside @0x1BEA90)
	//   m_numTriggerAreasActive +0x298 -> +0x346 (same body)
	// m_disabledMask trades places with m_partitionData; the two trigger fields
	// move into the block BFME appends past this header's 0x29C end. Which
	// members really fill the holes and the appended run is NOT proven.
	PartitionData*								m_partitionData;	///< our PartitionData	// swapped down from +0x1A4
  ...
  StealthUpdate*                m_stealth;
	...
	DisabledMaskType	m_disabledMask;	// retail +0x1A4 (swapped with m_partitionData)
	...
	char													_bfme_hole_triggerInfo[sizeof(TTriggerInfo) * MAX_TRIGGER_AREA_INFOS];	// moved to the tail
	...
	char													_bfme_hole_numTriggerAreas[1];	// moved to the tail
	Bool													m_singleUseCommandUsed;
	Bool													m_isReceivingDifficultyBonus;

	// ---- BFME appended block (see the note above) ----
	char _bfme_pad_to_triggerInfo[0x2D8 - 0x29C];
	TTriggerInfo									m_triggerInfo[MAX_TRIGGER_AREA_INFOS];	// retail +0x2D8
	char _bfme_pad_to_numTriggerAreas[0x346 - 0x300];
	Byte													m_numTriggerAreasActive;	// retail +0x346
