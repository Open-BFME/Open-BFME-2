...
{
	// BFME keeps IsValid up to date across the assignment where Zero Hour
	// leaves it alone: retail's VectorClass<ModelNodeClass>::operator= at
	// 0x0019E5B0 writes [esi+0Ch] three times -- false while the vector is
	// being rebuilt, true once it holds something, and true again for the
	// empty case, which is what the member's own comment describes ("valid
	// (even if empty)").  Those three stores are the whole difference between
	// 216 bytes and retail's 234.
	if (this != &vec) {
		Clear();
		IsValid = false;
