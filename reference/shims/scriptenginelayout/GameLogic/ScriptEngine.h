...
protected:
	...
	// BFME's ScriptEngine is 0x6868 bytes bigger ahead of this point than the
	// ZH header. Two independent pins agree on the delta:
	//   m_freezeByScript                      ours +0x10DBC -> retail +0x17624
	//   m_objectsShouldReceiveDifficultyBonus ours +0x10DCC -> retail +0x17634
	// (doEnableOrDisableObjectDifficultyBonuses @0x2F0840 pins the second).
	// Only the cumulative offset is proven - what fills the run is unknown -
	// so the padding sits immediately in front of the first pinned field.
	char _bfme_pad_pre_freezeByScript[0x6868];
