...
public:
	...
	// BFME's View carries two more virtuals ahead of setHeight than ZH's:
	// ControlBar::setSquishedControlBarConfig @0x49D0B0 reaches
	// TheTacticalView->setHeight through slot +0x40 where this header puts
	// it at +0x38. They are parked right here because that is the only
	// placement that leaves the View calls in the already-matched rows of
	// ControlBar.cpp untouched - the real names and position are unknown.
	virtual void _bfme_view_slot_before_setHeight_a( void ) { }
	virtual void _bfme_view_slot_before_setHeight_b( void ) { }
