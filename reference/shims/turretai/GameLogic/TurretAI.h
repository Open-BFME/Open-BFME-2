...
//
// BFME layout shim (only used by TurretAI.cpp). Two facts diverge from ZH:
//   - TurretAI's bases are the merged pooled-snapshot base + NotifyWeaponFiredInterface
//     (one vptr fewer than ZH's three bases), so m_data is at +0x8, m_angle at
//     +0x18 and m_pitch at +0x1C; deleteInstance comes from the merged base.
//   - TurretAIData is NOT polymorphic (no MemoryPoolObject vptr): the retail
//     FieldParse table at 0x00C9BA04 puts m_turnRate at +0x0 and the ctor
//     @0x18C300 zeroes +0x10..+0x1C / fills +0x20..+0x2C, i.e. the two per-slot
//     arrays hold WEAPONSLOT_COUNT+1 = 4 elements each and m_isAllowsPitch sits
//     at +0x56 (see friend_turnTowardsPitch @0x18C660).
//   - the eight booleans are plain Bytes at +0xA4..+0xAB, not one packed bitfield
//     byte: xfer @0x18C4B0 xfers them at +0xA4..+0xA9 and +0xAA/+0xAB, and
//     setTurretEnabled @0x18C930 reads m_enabled at +0xA8.
...
//-------------------------------------------------------------------------------------------------
// BFME's TurretAIData carries no vptr at all, so it cannot use the ZH
// MemoryPoolObject glue (which would add a virtual dtor). It keeps the
// magic-enum placement new/delete and a deleteInstance that mirror the
// non-pool-backed allocation retail uses.
class TurretAIData
{
public:
	Real						m_turnRate;
	...
	Real						m_turretFireAngleSweep[WEAPONSLOT_COUNT + 1];	///< if nonzero, sweep within +/- this angle range while firing
	Real						m_turretSweepSpeedModifier[WEAPONSLOT_COUNT + 1];	///< While sweeping, change your speed by this
	...
	enum TurretAIDataMagicEnum { TurretAIData_GLUE_NOT_IMPLEMENTED = 0 };
	...
	inline void *operator new(size_t s, TurretAIDataMagicEnum e DECLARE_LITERALSTRING_ARG2)
	{
		return ::operator new(s);
	}
	inline void operator delete(void *p, TurretAIDataMagicEnum e DECLARE_LITERALSTRING_ARG2)
	{
		::operator delete(p);
	}
 ...
 */
// BFME merges ZH's MemoryPoolObject and Snapshot into one base (see the
// TunnelTracker shim), so TurretAI carries one vptr fewer and every member
// drops a dword: getWhichTurretForWeaponSlot @0x26EBB0 reads m_angle at
// +0x18 and m_pitch at +0x1C where three polymorphic bases put them at
// +0x1C and +0x20. deleteInstance comes from that merged base. The eight
// state booleans are plain Bytes (m_playRotSound +0xA4 .. m_targetWasSetByIdleMood
// +0xAB), not a packed bitfield byte.
class TurretAI : public Snapshot, public NotifyWeaponFiredInterface
{
...
public:
	...
	// provided by BFME's merged pooled-snapshot base, not by MemoryPoolObject
	void deleteInstance();
