// Header shim: reference/open-bfme-1's WW3D2 camera.h with ONE BFME divergence
// -- CameraClass's ProjectionType member is not in front of Viewport.
//
// Read out of retail.  CameraClass::Device_To_View_Space at 0x00134650 has an
// instruction stream identical to ours for all 215 bytes except that every one
// of its eight member offsets is four LOWER: Viewport at 0xC8..0xD7 and
// ViewPlane at 0xD8..0xE7, where the Zero Hour layout puts them at 0xCC and
// 0xDC.
//
// The corroboration is a second body.  functions.csv claimed 0x00133D70 as
// Get_Horizontal_FOV on a masked single-hit placement, and it does match --
// but only by accident: `fld [ecx+0xE4] / fsub [ecx+0xDC]` is ViewPlane.Max.X
// minus Min.X under the Zero Hour layout and ViewPlane.Max.Y minus Min.Y under
// this one, so that address is really Get_Vertical_FOV.  Searching .text for
// the horizontal form under the corrected layout
// (`d9 81 e0 00 00 00 d8 a1 d8 00 00 00`) gives exactly one hit, at
// 0x00133D50 -- the sibling slot 32 bytes earlier.  Two functions landing in
// adjacent slots with the right names is not something a wrong layout produces.
//
// Scoped to camera.cpp by -Ireference/shims/bfmecamera on its `// cl:` line;
// the .cpp already includes "camera.h" first.
/*
...
protected:
	...
	// Projection is NOT the first data member in BFME.  Every field from
	// Viewport onwards sits four bytes lower than the Zero Hour layout puts it,
	// so the four bytes this enum occupies are not in front of them.  Where it
	// went is not something these bodies show, and the tail is the smallest
	// change that satisfies them -- if a body ever pins its real offset, move it
	// there rather than assuming this was right.
