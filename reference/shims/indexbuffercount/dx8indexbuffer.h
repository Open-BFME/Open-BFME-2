...
public:
	...
	// Also 32-bit in BFME: the WriteLockClass constructor at 0x00138790 reads
	// the count with a plain `mov eax,[eax+0Ch]` and doubles it, where an
	// unsigned short accessor forces a 16-bit load and a movzx.
...
protected:
	...
	// BFME widened this member to 32 bits.  Retail's
	// SortingIndexBufferClass constructor at 0x00138B20 zero-extends its
	// unsigned short argument once and then stores the whole register --
	// `movzx eax,word ptr [esp+14h]` ... `mov [esi+0Ch],eax` -- which a
	// 16-bit member cannot produce: against it MSVC emits a 16-bit load and
	// a 16-bit store and only then widens.  The offset of `type` does not
	// move; the two bytes were already padding.
