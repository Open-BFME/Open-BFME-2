...
//	HeroList m_heroList;		//< list of pointers to objects with radar icon representations
	...
	// BFME carries 0x108 more bytes ahead of m_radarWindow than ZH:
	// Radar::screenPixelToWorld @0x107C50 reads it at [this+0x1438] where
	// this header puts it at +0x1330. Only the cumulative offset is proven,
	// so the padding sits immediately in front of the field that pins it.
	char _bfme_pad_pre_radarWindow[0x108];
	GameWindow *m_radarWindow;						///< retail this+0x1438
