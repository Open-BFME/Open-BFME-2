...
protected:
  // BFME inlines this: retail's insert bodies allocate the node and construct
  // in place.  __forceinline alone will not do it -- MSVC will not inline a
  // function containing a try region at any /Ob level -- so the
  // _STLP_TRY/_STLP_UNWIND goes too, and retail's inlined copy has no unwind
  // handler around the construct either.  Same shape that landed
  // hashtable::_M_insert at 0x00620F60.
  __forceinline _Node* _M_create_node(const _Tp& __x)
  {
