// Header shim: reference/open-bfme-1's WWLib index.h with one BFME divergence.
// See the comment at IndexClass<INDEX,T>::Clear.  Pulled in by putting
// -Ireference/shims/bfmeindex first on a unit's `// cl:` line AND including
// "index.h" from the .cpp before any header that would reach the reference
...
{
	// BFME frees this table with the SCALAR deallocator.  INIClass::Clear at
	// 0x00616C60 inlines this body and its call reaches ??3@YAXPAX@Z
	// (0x0002FD60), not ??_V@YAXPAX@Z (0x0002FD80); the two are separate
	// byte-verified bodies in this image, so it is not an ICF fold.  That
	// displacement is the whole of the evidence and it only covers this site:
	// Increase_Table_Size's `delete [] IndexTable` below is left alone until a
	// body that inlines IT byte-proves the same thing.
	delete IndexTable;
	IndexTable = 0;
	IndexCount = 0;
	IndexSize = 0;
