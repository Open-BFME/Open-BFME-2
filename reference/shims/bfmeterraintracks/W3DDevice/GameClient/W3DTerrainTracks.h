// Header shim: the Zero Hour W3DTerrainTracks.h with one extra dword at the
// head of TerrainTracksRenderObjClass.  See the comment there.
// Scoped by -Ireference/shims/bfmeterraintracks on the unit's `// cl:` line.
/*
...
protected:
	// BFME has four bytes here that the Zero Hour class does not.  Not a
	// guess about what they hold -- a consequence of two facts that are
	// each byte-proven elsewhere.  sizeof(RenderObjClass) is 0xC4, not
	// 0xC8 (see reference/shims/bfmerendobj/rendobj.h, three classes
	// agreeing), and Get_Obj_Space_Bounding_Sphere at 0x001A54C0 and
	// Get_Obj_Space_Bounding_Box at 0x0019BFA0 reach m_boundingSphere and
	// m_boundingBox at the offsets the OLD base produced.  Both cannot be
	// true unless this class carries one extra dword ahead of them.
	// Nothing here says what it is, so it is not named.
	char		_bfme_unk_head[4];
