// Header shim: the WW3D2 mapper.h with one extra dword at the head of
// GridTextureMapperClass.  See the comment there.  Scoped by
// -Ireference/shims/bfmemapper on the unit's `// cl:` line; mapper.cpp
// already includes "mapper.h" first.
/*
...
protected:	
	...
	// BFME has one more dword here than Zero Hour does.  Read out of the two
	// GridWSEnvMapperClass constructors: 0x00186DA0 stores LastFrame at
	// [esi+0x20], Offset at [esi+0x24] and the axis at [esi+0x34] where this
	// build stores them at 0x1C, 0x20 and 0x30, while [esi+4] (the ref count)
	// and [esi+8] (Stage) agree -- so the four bytes are after Stage and before
	// Sign, which is the head of THIS class rather than of TextureMapperClass.
	// What they hold is not visible from these bodies, so it is not named.
	char				_bfme_unk_grid_head[4];
