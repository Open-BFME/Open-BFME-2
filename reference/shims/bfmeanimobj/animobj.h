// Header shim: animobj.h with 4 unknown bytes inserted
// before `int								CurMotionMode;`.  See the comment there for the evidence.
// Scoped by -Ireference/shims/bfmeanimobj on the unit's `// cl:` line.
/*
...
protected:
	...
	// This shim exists only to make the unit resolve animobj.h to the
	// reference copy instead of the older reference/shims/sweep one.  The
	// sweep header has no _bfme_a3o_v0 above, and that dword is what puts
	// CurMotionMode at retail +0x100: Set_Animation() at 0x001A4BD0 stores
	// its mode there, and the sweep layout put it at +0xFC.  No padding of
	// this shim's own -- an earlier draft added four bytes here and moved the
	// member to +0x104, which is the same dword counted twice.
	int								CurMotionMode;
