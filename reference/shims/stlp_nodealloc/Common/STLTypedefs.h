...
// BFME shim: the one line below, guarded. Zero Hour forces STL containers onto
// plain new/delete; retail BFME's container code deallocates with the node
// size (push 0x18; call 0x0082E5F0, which buckets by (n-1)>>3 into the
// free-list array at 0x0130B1C0), which is STLport's _Node_alloc. _alloc.h
// selects __new_alloc whenever _STLP_USE_NEWALLOC is defined at all, so a TU
// that needs the node allocator defines BFME_STLP_NODE_ALLOC and puts this
// directory ahead of the reference include path. Everything else here is the
// reference header verbatim.
