...
private:
	...
	// BFME keeps ONE weapon-recoil vector here, not ZH's WEAPONSLOT_COUNT (3).
	// Each WeaponRecoilInfoVec is 12 bytes, and retail puts m_renderObject at
	// this+0x28 where our layout puts it at +0x40 - exactly two vectors'
	// worth. Pinned by three retail bodies that all read m_renderObject:
	// clientOnly_getRenderObjBoundBox @0x75C470, getCurrentWorldspaceClient
	// BonePositions and clientOnly_getRenderObjBoneTransform.
	WeaponRecoilInfoVec						m_weaponRecoilInfoVec[1];
	...
	// ...but the bytes come back straight after it: an already-matched row in
	// this TU reads a field at this+0x58 and retail agrees with our unshifted
	// offset there, so BFME removes 0x18 before m_renderObject and adds 0x18
	// again right after it. Only those two cumulative offsets are proven; what
	// actually occupies the 0x18 is unknown.
	char												_bfme_pad_post_renderObject[0x18];
