...
public:
	...
	// SHIM: BFME widened openFile to four parameters across the whole file-system
	// family and put the wide form at the slot after the narrow one. Declared
	// widest-first because MSVC lays out a run of same-name virtual overloads in
	// reverse declaration order, which is what keeps the narrow form at slot 2
	// (ArchiveFileSystem::openFile reaches it through [eax+0x08]) and puts the
	// wide one at slot 3 ([eax+0x0c]).
	virtual File*					openFile( const Char *filename, Int access, Int a3, Int a4 ) = 0;
