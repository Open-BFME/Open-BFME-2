...
private:
  ...
  // BFME's hashtable::_M_insert INLINES this: retail allocates the node with
  // `push 0 / push 0Ch / call` and constructs in place, where our build emits a
  // call to _M_new_node.  __forceinline is what closes that; MSVC will not
  // inline it on its own even at /Ob2.
  // ...and the try region has to go with it.  MSVC will not inline a function
