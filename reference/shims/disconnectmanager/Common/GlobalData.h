...
// TU-scoped stand-in for Common/GlobalData.h, carrying the three network timing
// members this TU reads at BFME's offsets.
//
// Retail pins three of them: m_networkKeepAliveDelay at +0xCB4,
// m_networkPlayerTimeoutTime at +0xCBC and m_networkDisconnectScreenNotifyTime
// at +0xCC0 (native_connection_timing.cpp). Keep-alive to player-timeout is
// eight bytes, so BFME has exactly ONE member between them where the reference
// has two -- and the one it dropped is m_networkRunAheadSlack, which is what a
// build with no run-ahead would drop. Removing it and padding 940 bytes in front
// of m_networkKeepAliveDelay puts all four members, including
// m_networkDisconnectTime at +0xCB8, exactly where retail has them.
//
// Everything after the padding shifts; nothing matched in this TU touches it.
...
public:
  ...
  //	Int m_cheaterHasBeenSpiedIfMyLowestBitIsTrue; ///< says it all.. this lives near other "colors" cause it is masquerading as one
	...
	// Puts m_networkKeepAliveDelay at +0xCB4, where retail has it.
	char m_padToBFMENetworkTiming[0xCB4 - 0x908];
	...
	// BFME has no m_networkRunAheadSlack -- it has no run-ahead at all.
