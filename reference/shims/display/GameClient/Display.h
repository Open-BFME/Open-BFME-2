...
public:
	...
	// BFME's Display has three more virtuals ahead of getWidth than ZH's:
	// ControlBarResizer::sizeWindowsAlt @0x4AB1A0 calls getWidth through slot
	// 0x2C and getHeight through 0x30, where this header puts them at 0x20 and
	// 0x24. The pair stays one slot apart in both, so the three added methods
	// all sit before getWidth; exactly where, and what they do, is unknown, so
	// they are stubs placed immediately in front of the pin.
	virtual void _bfme_display_slot8( void ) { }
	virtual void _bfme_display_slot9( void ) { }
	virtual void _bfme_display_slot10( void ) { }
