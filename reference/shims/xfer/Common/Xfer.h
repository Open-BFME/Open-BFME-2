// TU-scoped shim: Zero Hour's Xfer.h with xferUser moved to vtable slot 9,
// which is where BFME dispatches it. See the note at the declaration.
...
public:
	...
	// BFME dispatches xferUser through vtable slot 9. INI::readLine hands every
	// line it reads to it -- push length, push buffer, call [vtbl+0x24] -- so the
	// index is proven even though nothing else about this vtable is. Zero Hour
	// puts xferUser at slot 46 and slot 9 at "skip", which takes one argument
	// where retail's takes two, so the two vtables are genuinely different rather
	// than merely shifted.
	//
	// Declared here purely to land on index 9. The slots after it are Zero Hour's
	// and are almost certainly not BFME's; only the TUs that call xferUser may use
	// this header.
	...
	// (xferUser is declared earlier in this shim -- see the note there.)
