// TU-scoped shim: Zero Hour's Water.h with BFME's WaterTransparencySetting
// tail. Nothing else differs.
...
class WaterTransparencySetting : public Overridable
{
	...
	public:
		...
		// BFME replaces Zero Hour's five skybox texture names with a river multiplier
	// and two more words. sizeof is 0x40 here against 0x48 there, which is what
	// parseWaterTransparencyDefinition allocates (push 0x40), and the field table
	// at 0x010F6CD0 puts RiverTransparencyMultiplier at 0x34 as its last entry.
	// The two words after it take no INI field, so they are runtime-only state.
	Real m_riverTransparencyMultiplier;		// 0x34
	Int _bfme_unknown38;					// 0x38
	Int _bfme_unknown3c;					// 0x3c
	...
	public:
		WaterTransparencySetting()
		{
			...
			// no skybox texture defaults: BFME has no such members here
