...
// TU-scoped stand-in for GameNetwork/NetworkDefs.h, scoped through
// /Ireference/shims/firewall. Only the magic number differs from the reference.
...
// BFME stamps 0xF00E, not the reference's 0xF00D. It is the single byte between
// this TU's sendToManglerFromPort and retail's, and it is the only place the
// magic number survives at all -- BFME dropped the field from
// TransportMessageHeader entirely and validates those packets by CRC instead.
static const UnsignedShort GENERALS_MAGIC_NUMBER = 0xF00E;
