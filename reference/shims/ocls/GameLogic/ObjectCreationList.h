...
// BFME2 layout: ObjectCreationListStore::addObjectCreationNugget (0x001F0AC4,
// called via a tail jmp from the matched ObjectCreationList::addObjectCreationNugget
// @ 0x001F0BDA) reads m_nuggets at this+0x24 ("add ecx,0x24"; the reference
// layout puts it at +0x14 -- a lone byte differs, an immediate, everything
// else in the 16-byte body is identical). That is 0x10 unaccounted bytes
// ahead of m_nuggets, placed here as a named gap between m_ocls and m_nuggets
// since the one body that proves anything only proves m_nuggets moved, not
// which field explains it. ModuleFactory has a similarly-shaped but
// differently-sized gap (+4 vs this +0x10) despite also deriving from
// SubsystemInterface, which rules out "SubsystemInterface grew" as a shared
// cause for either.
//
// UNCONFIRMED beyond that one body: a second-body check via
// tools/shift_place.py surfaced a uniform +4 hit for
// ??0ObjectCreationListStore@@QAE@XZ (the ctor) that would place m_nuggets at
// +0x18, not +0x24 -- directly conflicting with the addObjectCreationNugget
// reading. Both the map ctor and the _Vector_base<T*> ctor calls that hit
// depends on are themselves masked-out relocations (call targets), and
// _Vector_base<T*>'s body is identical for any pointer T, so a uniform-shape
// match against some OTHER SubsystemInterface + map<NameKeyType,V> + vector
// class is plausible and was not ruled out. Treat the ctor hit as refuted,
// not confirming, until a genuinely independent address (not found via
// masked shape search) checks out.
///////////////////////////////////////////////////////////////////////////////////////////////////
...
private:
	...
	// 0x10 bytes retail carries between m_ocls and m_nuggets that the reference
	// layout does not (see the note at the top of this file). Name is neutral;
	// no behavior for it is reconstructed here.
	char _bfme_unk_before_nuggets[0x10];
