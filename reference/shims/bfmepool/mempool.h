...
**
**********************************************************************************************/
// BFME spin-locks its object pools.  Free_Object_Memory at 0x00196470 does
// `lea edi,[esi+10h] / mov ecx,edi / call 0006577F` before touching the free
// list and stores a plain zero through edi at the end; 0x10 is exactly where a
// fifth member lands after the four in ObjectPoolClass.  The callee's body is
// `lock bts dword ptr [ebx],0` with a thread yield on contention.
//
// The ADDRESS is self-verifying -- retail encodes it in the displacement -- but
// the NAME is ours.  It is a no-argument thiscall whose `this` is used directly
// as the flag word, so it is not Zero Hour's FastCriticalSectionClass::LockClass
// constructor, which takes the section by reference and would have pushed it.
// Spelled BFME-prefixed so nobody mistakes our reading for a recovered symbol.
class BFMEPoolCriticalSection
{
public:
	volatile unsigned Flag;
	// declared, not defined, so the call goes out of line; throw() so /EHsc
	// does not wrap the guard below in an unwind region retail does not have.
	void Lock() throw();
};

// The release has to be a DESTRUCTOR, not a trailing store.  Retail publishes
// the new free-list head and only then clears the flag; written as a plain
// assignment at the end of the function MSVC sinks the head store past it and
// the last two instructions come out swapped.
class BFMEPoolLock
{
	BFMEPoolCriticalSection &cs;
public:
	BFMEPoolLock(BFMEPoolCriticalSection &section) : cs(section) { section.Lock(); }
	~BFMEPoolLock() { cs.Flag = 0; }
private:
	BFMEPoolLock(const BFMEPoolLock &);
	BFMEPoolLock &operator=(const BFMEPoolLock &);
};
...
protected:
	...
	BFMEPoolCriticalSection ObjectPoolCS;	// 0x10
...
{
	BFMEPoolLock lock(ObjectPoolCS);
	WWASSERT(obj != NULL);
